...
// =============================================================================
// TAPERED EVALUATION + PAWN STRUCTURE
// Adapted from Stockfish 11. Scaled to internal units (1cp = 8 units).
// =============================================================================

var g_passedMG = [0,  40,  96,  80, 456, 1304, 1792];
var g_passedEG = [0, 144, 184, 248, 496, 1336, 1744];

function GamePhase() {
    var p =   g_pieceCount[pieceKnight]         + g_pieceCount[pieceKnight | colorWhite]
            + g_pieceCount[pieceBishop]         + g_pieceCount[pieceBishop | colorWhite]
            + 2 * (g_pieceCount[pieceRook]      + g_pieceCount[pieceRook   | colorWhite])
            + 4 * (g_pieceCount[pieceQueen]     + g_pieceCount[pieceQueen  | colorWhite]);
    return p > 24 ? 24 : p;
}
...
function FileHasPawn(file, isWhite) {
    for (var r = 0x20; r <= 0x90; r += 0x10) {
        var p = g_board[r | file];
        if ((p & 0x7) != piecePawn) continue;
        if (isWhite  && (p & colorWhite) != 0) return true;
        if (!isWhite && (p & colorWhite) == 0 && p != 0) return true;
    }
    return false;
}

function PawnStructureBonus(phase) {
    var ISO_MG = 40,  ISO_EG = 120;
    var DBL_MG = 88,  DBL_EG = 448;
    var mgAcc = 0, egAcc = 0;
    var pieceIdx, sq, file, row, rankIdx, r, p, lOk, rOk, doubled, passed;

    // White pawns
    pieceIdx = (colorWhite | piecePawn) << 4;
    sq = g_pieceList[pieceIdx++];
    while (sq != 0) {
        file    = sq & 0x0F;
        row     = sq & 0xF0;
        rankIdx = (0x90 - row) >> 4;

        lOk = (file > 0x04) && FileHasPawn(file - 1, true);
        rOk = (file < 0x0B) && FileHasPawn(file + 1, true);
        if (!lOk && !rOk) { mgAcc -= ISO_MG; egAcc -= ISO_EG; }

        doubled = false;
        for (r = 0x20; r < row; r += 0x10) {
            p = g_board[r | file];
            if ((p & 0x7) == piecePawn && (p & colorWhite) != 0) { doubled = true; break; }
        }
        if (doubled) { mgAcc -= DBL_MG; egAcc -= DBL_EG; }

        passed = true;
        for (r = 0x20; r < row && passed; r += 0x10) {
            p = g_board[r | file];
            if ((p & 0x7) == piecePawn && (p & colorWhite) == 0 && p != 0) { passed = false; break; }
            if (file > 0x04) { p = g_board[r | (file-1)]; if ((p & 0x7) == piecePawn && (p & colorWhite) == 0 && p != 0) { passed = false; break; } }
            if (file < 0x0B) { p = g_board[r | (file+1)]; if ((p & 0x7) == piecePawn && (p & colorWhite) == 0 && p != 0) { passed = false; break; } }
        }
        if (passed && rankIdx >= 1 && rankIdx <= 6) { mgAcc += g_passedMG[rankIdx]; egAcc += g_passedEG[rankIdx]; }

        sq = g_pieceList[pieceIdx++];
    }

    // Black pawns
    pieceIdx = piecePawn << 4;
    sq = g_pieceList[pieceIdx++];
    while (sq != 0) {
        file    = sq & 0x0F;
        row     = sq & 0xF0;
        ...
        lOk = (file > 0x04) && FileHasPawn(file - 1, false);
        rOk = (file < 0x0B) && FileHasPawn(file + 1, false);
        if (!lOk && !rOk) { mgAcc += ISO_MG; egAcc += ISO_EG; }

        doubled = false;
        for (r = 0x90; r > row; r -= 0x10) {
            p = g_board[r | file];
            if ((p & 0x7) == piecePawn && (p & colorWhite) == 0 && p != 0) { doubled = true; break; }
        }
        if (doubled) { mgAcc += DBL_MG; egAcc += DBL_EG; }

        passed = true;
        for (r = 0x90; r > row && passed; r -= 0x10) {
            p = g_board[r | file];
            if ((p & 0x7) == piecePawn && (p & colorWhite) != 0) { passed = false; break; }
            if (file > 0x04) { p = g_board[r | (file-1)]; if ((p & 0x7) == piecePawn && (p & colorWhite) != 0) { passed = false; break; } }
            if (file < 0x0B) { p = g_board[r | (file+1)]; if ((p & 0x7) == piecePawn && (p & colorWhite) != 0) { passed = false; break; } }
        }
        if (passed && rankIdx >= 1 && rankIdx <= 6) { mgAcc -= g_passedMG[rankIdx]; egAcc -= g_passedEG[rankIdx]; }
    ...
    return Taper(mgAcc, egAcc, phase);
}
...
function Evaluate() {
    ...
    if (g_pieceCount[pieceBishop] >= 2)             evalAdjust -= bpBonus;
    if (g_pieceCount[pieceBishop | colorWhite] >= 2) evalAdjust += bpBonus;

    // Pawn structure — isolated, doubled, passed (Stockfish 11)
    evalAdjust += PawnStructureBonus(phase);
