...
var g_engineInfo = {
    1: { name: "FM Beginner", sub: "(1200+ Elo)" },
    2: { name: "FM Training", sub: "(1800+ Elo)" },
...
function UpdateEvalBar(scoreInternal) {
    ...
    var cp      = scoreInternal / 8;
    var clamped = cp < -500 ? -500 : cp > 500 ? 500 : cp;
    var whitePct = 50 + (clamped / 500) * 35;
    ...
    var pawn = cp / 100;
    var sign = pawn >= 0 ? '+' : '';
    label.textContent = sign + pawn.toFixed(1);
}
